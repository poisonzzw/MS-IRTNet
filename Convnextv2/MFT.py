...
class CrossPath(nn.Module):
    def __init__(self, dim, reduction=1, num_heads=None, norm_layer=nn.LayerNorm):
        ...
        self.end_proj1 = nn.Linear(dim // reduction * 4, dim)
    ...
    def forward(self, x1, x2):
        ...
        fusion = torch.cat((v1, v2), dim=-1)
        raw_fusion = torch.cat((y1,y2), dim=-1)

        fusion = fusion+raw_fusion
        fusion = self.end_proj2(fusion)
        out_x1 = self.norm1(fusion)
